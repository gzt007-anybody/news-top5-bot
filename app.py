...
def get_naver_news(query, start=1, display=100):
    ...
    try:
        ...
        st.error("네이버 뉴스 API 연결 시간이 초과되었습니다.")
        return []
    ...
    if response.status_code != 200:
        ...
        st.error(
            f"네이버 뉴스 API 오류: {response.status_code}"
        )

        st.markdown("#### 🔎 오류 상세 진단")

        st.write(
            f"**오류코드:** {error_code if error_code else '확인 불가'}"
        )

        st.write(
            f"**오류내용:** {error_message if error_message else '확인 불가'}"
        )

        st.caption(
            "※ 보안을 위해 Client ID와 Client Secret 실제 값은 화면에 표시하지 않습니다."
        )

        # 401 전용 안내
        if response.status_code == 401:
            st.warning(
                "401 오류는 대부분 네이버 API 인증 문제입니다.\n\n"
                "확인할 항목:\n"
                "1. 이 Streamlit 앱의 Secrets에 NAVER_CLIENT_ID / NAVER_CLIENT_SECRET이 들어있는지\n"
                "2. 네이버 개발자센터의 해당 애플리케이션에서 검색 API 사용이 가능한지\n"
                "3. Client Secret이 재발급된 적이 있는지\n"
                "4. Secrets 값 앞뒤에 공백이나 줄바꿈이 들어가 있지 않은지"
            )
